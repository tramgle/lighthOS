...
/* system(cmd) runs `cmd` via `/bin/shell -c` so callers get full
   shell syntax: redirection, pipes, background, quoting, $VAR.
   NULL cmd returns 1 (POSIX: "a shell is available"). */
int system(const char *cmd) {
    if (!cmd) return 1;
    /* Copy into a mutable buffer so execve gets a stable pointer
       even if the caller's cmd is in a short-lived arena. */
